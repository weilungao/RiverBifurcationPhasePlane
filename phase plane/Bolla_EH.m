...
f_dD1_dt=@(D1, D2) (B1*max(zeros(size(D1)), (((beta1*D1.^(3/2)./(beta1*D1.^(3/2)+beta2*D2.^(3/2)))*(D0*B0*C0)./(D1*B1*C1)).^2*theta0-theta_cr)).^n-...
                              B0*min(ones(size(D1)), max(zeros(size(D1)), (((beta1*D1.^(3/2)./(beta1*D1.^(3/2)+beta2*D2.^(3/2)))+(2*alpha*r/sqrt(theta0)*(D1-D2)/B0)))))* (theta0-theta_cr).^n);
                          
f_dD2_dt=@(D1, D2) (B2*max(zeros(size(D1)), (((beta2*D2.^(3/2)./(beta1*D1.^(3/2)+beta2*D2.^(3/2)))*(D0*B0*C0)./(D2*B2*C2)).^2*theta0-theta_cr)).^n-...
                              B0*min(ones(size(D1)), max(zeros(size(D1)), (((beta2*D2.^(3/2)./(beta1*D1.^(3/2)+beta2*D2.^(3/2)))-(2*alpha*r/sqrt(theta0)*(D1-D2)/B0)))))* (theta0-theta_cr).^n);

%dDi/dt                       
A1=[0:0.04*D1_max:2*D1_max];
A2=[0:0.04*D2_max:2*D2_max];
...
for i=1:length(A1)
    for j=1:length(A2)
        ...
        dD1_dt(j,i)=m*sqrt(R*g*d50^3)/B1/L1*(B1*max(0, (((beta1*D1^(3/2)/(beta1*D1^(3/2)+beta2*D2^(3/2)))*(D0*B0*C0)/(D1*B1*C1))^2*theta0-theta_cr))^n-...
                              B0*min(1, max(0, (((beta1*D1^(3/2)/(beta1*D1^(3/2)+beta2*D2^(3/2)))+(2*alpha*r/sqrt(theta0)*(D1-D2)/B0)))))* (theta0-theta_cr)^n);
...
%Qsi=0
f_Qs1=@(D1, D2) (((beta1*D1.^(3/2)./(beta1*D1.^(3/2)+beta2*D2.^(3/2)))+(2*alpha*r/sqrt(theta0)*(D1-D2)/B0)));
f_Qs2=@(D1, D2) (((beta2*D2.^(3/2)./(beta1*D1.^(3/2)+beta2*D2.^(3/2)))-(2*alpha*r/sqrt(theta0)*(D1-D2)/B0)));
...
f_dDi_dt = @root2d;
x0 = [0.1, 0.1];
x = fsolve(f_dDi_dt ,x0);
if theta0>theta0_cr
    scatter(x(1),x(2), marker_size,'bo','LineWidth',marker_lw)
else
    scatter(x(1),x(2), marker_size,'bo','filled')
end
...
x = fsolve(f_dDi_dt ,x0);
if x(1)/x(2)-1>RD_cr
    scatter(x(1),x(2), marker_size,'k^','filled')
end
...
x = fsolve(f_dDi_dt ,x0);
if x(2)/x(1)-1>RD_cr
    scatter(x(1),x(2), marker_size,'r^','filled')
end
...
f_dD1_dt_D2_0=@(D1) (B1*max(0, (((beta1*D1^(3/2)/(beta1*D1^(3/2)+beta2*D2^(3/2)))*(D0*B0*C0)/(D1*B1*C1))^2*theta0-theta_cr))^n-...
                              B0*min(1, max(0, (((beta1*D1^(3/2)/(beta1*D1^(3/2)+beta2*D2^(3/2)))+(2*alpha*r/sqrt(theta0)*(D1-D2)/B0)))))* (theta0-theta_cr)^n);
x = fzero(f_dD1_dt_D2_0, D1_max);
scatter(x, 0, marker_size, 'ks','LineWidth',marker_lw)
...
f_dD2_dt_D1_0=@(D2) (B2*max(0, (((beta2*D2^(3/2)/(beta1*D1^(3/2)+beta2*D2^(3/2)))*(D0*B0*C0)/(D2*B2*C2))^2*theta0-theta_cr))^n-...
                              B0* min(1, max(0, (((beta2*D2^(3/2)/(beta1*D1^(3/2)+beta2*D2^(3/2)))-(2*alpha*r/sqrt(theta0)*(D1-D2)/B0)))))* (theta0-theta_cr)^n);
x = fzero(f_dD2_dt_D1_0, D1_max);
scatter(0, x, marker_size, 'rs','LineWidth',marker_lw)
...
box on
FontSize1=15;
% legend([h1, h2, h3, h4], 'd{\it D}_1/d{\it t}=0', 'd{\it D}_2/d{\it t}=0', '{\it Q_{s}}_1=0', '{\it Q_{s}}_2=0', 'FontSize', FontSize1, 'FontName', 'Times New Roman');

fig_name=strcat('Bolla_EH_theta_0=', num2str(theta0), '.tif');
